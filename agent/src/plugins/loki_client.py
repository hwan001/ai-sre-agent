...
class LogQLQuery(BaseModel):
    ...
    start: Optional[datetime] = Field(None, description="Query start time")
    end: Optional[datetime] = Field(None, description="Query end time")
    limit: int = Field(1000, description="Maximum number of log entries")
    direction: str = Field("backward", description="Query direction: forward or backward")
...
class LogStream(BaseModel):
    ...
    def from_loki_stream(cls, stream_data: dict) -> LogStream:
        ...
        entries = [
            LogEntry.from_loki_entry(entry) 
...
class LokiClient:
    ...
    def _generate_mock_query_result(self, query: str, time_window_minutes: int = 30) -> LokiQueryResult:
        ...
        mock_entries = [
            LogEntry(
                ...
                labels={"namespace": "production", "pod": "app-pod", "container": "main"}
            )
            for i in range(1, 6)
        ]
        
        mock_stream = LogStream(
            stream={"namespace": "production", "pod": "app-pod"},
            values=mock_entries
        )
...
class LokiTools:
    ...
    def __init__(self, loki_client: Optional[LokiClient] = None, mock: bool = False):
        """
            ...
            async with client:
                ...
                for entry in entries:
                    ...
                    if "exception" in line_lower:
                        error_patterns["exceptions"] = error_patterns.get("exceptions", 0) + 1
                    if "failed" in line_lower:
                        error_patterns["failures"] = error_patterns.get("failures", 0) + 1
    ...
    async def get_application_logs(
                ...
                "logs": [
                    {
                        ...
                        "labels": {"app": app_label, "namespace": namespace or "default"}
                    },
                    {
                        "timestamp": (datetime.now() - timedelta(minutes=10)).isoformat(),
        ...
        try:
            ...
            logger.error("Application log retrieval failed", error=str(e), app=app_label)
            return {
                "tool": "get_application_logs",
                "error": str(e),
                "app_label": app_label,
...
_loki_tools: Optional[LokiTools] = None
